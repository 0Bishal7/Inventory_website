...
def get_faq(mainCursor):
    mainCursor.execute('SELECT * FROM inventory_website.faq WHERE is_trash=0', ())
...
def change_faq_status(mainCursor):
    ...
    mainCursor.execute('UPDATE inventory_website.faq SET status = %s WHERE id = %s', (status, id))
    return jsonify({
        "res": 1,
...
def delete_faq(mainCursor):
    ...
    mainCursor.execute('UPDATE inventory_website.faq SET is_trash = %s WHERE id = %s', (is_trash, id))
    return jsonify({
        "res": 1,
...
def add_faq(mainCursor):
    ...
    mainCursor.execute(
        "INSERT INTO inventory_website.faq (question, answer) VALUES (%s, %s)",
        (
            data['question'],
            data['answer']
        )
