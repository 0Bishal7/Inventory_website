...
def get_slider(mainCursor):
    mainCursor.execute('SELECT * FROM inventory_website.slider WHERE is_trash=0', ())
...
def change_slider_status(mainCursor):
    ...
    mainCursor.execute('UPDATE inventory_website.slider SET status = %s WHERE id = %s', (status, id))
    return jsonify({
        "res": 1,
...
def delete_slider(mainCursor):
    ...
    mainCursor.execute('UPDATE inventory_website.slider SET is_trash = %s WHERE id = %s', (is_trash, id))
    return jsonify({
        "res": 1,
...
def add_slider(mainCursor):
    ...
    mainCursor.execute(
        "INSERT INTO inventory_website.slider (title, description, image_path, button_link, other_link) VALUES (%s, %s, %s, %s, %s)",
        (
            data_text['title'],
